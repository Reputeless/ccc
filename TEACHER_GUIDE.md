...
- `CCC` は、授業中の一斉提出システムというより、学生が自習や復習で使う演習環境に向いています
- 内部構成や仕様の概要は、必要に応じて [GitHub リポジトリ](https://github.com/Reputeless/ccc) を参照してください
...
この節のパーミッションは、現在の動作確認環境でうまく動いている設定例です。  
サーバーの種類や設定によっては異なる場合があるため、うまく動かない場合は契約中のサーバー仕様も確認してください。

- パーミッションは、サーバー上でファイルやフォルダをどのように読めるかを決める設定です
