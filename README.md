...
- 列は `problem.json` の主要項目に寄せ、`id / number / title / lecture / difficulty / profileId / publishedAt / examples / 結果 / 詳細` を表示する
...
- `学習記録` パネルからは、最後に開いた問題への復帰や、ローカルに保存された学習記録の消去も行える
...
- スマホからの利用はできるとうれしい
- ただし初期版では、まずデスクトップ中心で設計する
- スマホで閲覧・判定が一応できることを目標にする
- 専用の強い最適化は必須ではないが、通学中でも使いやすいことは意識する
