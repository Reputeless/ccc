...
- 講義回、難易度、解いたかどうか、理解度で問題一覧を絞り込める
...
- 現在の実装:
  - 講義回の昇順
  - 講義回の降順
  - 同じ講義回の中は `number`、未設定時は `id` で比較する
- 将来的な候補:
  - 問題番号の辞書順昇順
  - 問題番号の辞書順降順
...
- 問題が存在しない、または未公開の場合は、その旨を表示する専用画面を出し、一覧へ戻るリンクを置く
...
- `Accepted`: `Accepted (N ケースすべて通過)` のように表示できる
- `Wrong Answer`: 最初に失敗した 1 ケースについて、入力、期待出力、実際の出力を表示する
- `Compile Error`: コンパイルエラーメッセージを表示する
- `Runtime Error`: 実行時エラーであることを表示する
- `Timeout`: タイムアウトであることを表示する
- `Warning`: 判定には影響させず、個別問題ページの結果欄に表示する
...
- 基本は空のコード欄を使う
- `Tab` キーでは実際のタブ文字 `\t` を入力する
- タブ幅の見た目は既定で空白 4 文字分とする
...
- 入出力例や期待出力など、ジャッジに影響する変更は `problemId` を変更して新規問題として扱う
...
- Wandbox へのリクエスト構築
- 出力比較
- `Accepted` / `Wrong Answer` / `Compile Error` / `Runtime Error` / `Timeout` への分類
- 学習支援向けの結果整形
...
- 案 A: 送信前にフロントでも長さを確認し、バックエンドでも再確認する
- 案 B: バックエンドだけで確認する

#### 2. Wandbox 障害時の案内

- 案 A: 毎回固定メッセージを返す
- 案 B: 接続失敗とレート制限らしき失敗を少し分けて文言を返す

Markdown 変換場所と問題本文 API の返し方は、初期版では「サーバ側で HTML に変換し、`bodyHtml` を返す」で確定とする。

API 失敗時の画面文言は、初期版ではフロントエンド側で定型文を集中管理する。
...
- Wandbox と疎通する
- `判定する` を実装する
- `Accepted` / `Wrong Answer` / `Compile Error` / `Runtime Error` / `Timeout` を表示する
- コンパイル警告を結果欄に表示する
